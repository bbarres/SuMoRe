...
IDH_coex<-table(data_pays$coexistence_pays,
                data_pays$IDH_classes2)
#wer remove the NA class
IDH_coex<-IDH_coex[-c(2),]
colnames(IDH_coex)<-c("[<0.8]","[0.8-0.9]","[0.9-1]")
...
text(temp,par("usr")[1]-10,labels=names(effectif),srt=0,
...
IDH_PPPsurv<-table(data_pays$q136_q137_UsagePPP,
                   data_pays$IDH_classes2)
colnames(IDH_PPPsurv)<-c("[<0.8]","[0.8-0.9]","[0.9-1]")
...
##############################################################################/
#Figure Box A: Continent vs composition of NRML####
##############################################################################/

colovec<-brewer.pal(7,"Dark2")[7:1]

#formating the table necessary for the ploting
conti_coex<-table(data_pays$coexistence_pays,
                  data_pays$continent)
#wer remove the NA class
conti_coex<-conti_coex[-c(2),]
row.names(conti_coex)<-c("Academic","Academic\nPrivate",
                         "Private","Academic\nGovernmental",
                         "Governmental","Academic\nGovernmental\nPrivate",
                         "Governmental\nPrivate")
#reordering the lines
conti_coex<-conti_coex[c(3,1,5,7,4,2,6),]
#reordering the columns
conti_coex<-conti_coex[,c(1,2,3,4,6,5)]
effectif<-colSums(conti_coex)
effectif
conti_coex<-prop.table(conti_coex,margin=2)*100
conti_coex

def.par<-par(no.readonly=TRUE)
layout(cbind(1,2),widths=c(9,3))  #put legend on the right margin of the chart
op<-par(mar=c(6.1,5.1,2,0))
temp<-barplot(conti_coex,col=colovec,border=NA,axes=FALSE,
              axisnames=FALSE,space=0.7,xpd=FALSE)
axis(1,at=temp,labels=FALSE,lwd=4,font=2,
     cex.axis=1.1,padj=0.1,xpd=TRUE,las=1)
text(temp,par("usr")[1]-6,labels=names(effectif),srt=25,adj=c(1,NA),
     xpd=TRUE,cex=1.2,font=2)
axis(2,lwd=4,font=2,cex.axis=1.2,las=1)
box(bty="l",lwd=4)
title(main=NULL,xlab="Continent",ylab="",cex.lab=2,
      line=4.5,font.lab=2)
title(main=NULL,xlab="",ylab="% of countries",cex.lab=2,
      line=3,font.lab=2)
text(temp[1],104,paste("n=",effectif[1],sep=""),font=3,cex=1.2,xpd=TRUE)
text(temp[2],104,paste("n=",effectif[2],sep=""),font=3,cex=1.2,xpd=TRUE)
text(temp[3],104,paste("n=",effectif[3],sep=""),font=3,cex=1.2,xpd=TRUE)
text(temp[4],104,paste("n=",effectif[4],sep=""),font=3,cex=1.2,xpd=TRUE)
text(temp[5],104,paste("n=",effectif[5],sep=""),font=3,cex=1.2,xpd=TRUE)
text(temp[6],104,paste("n=",effectif[6],sep=""),font=3,cex=1.2,xpd=TRUE)

par(mar=c(4.1,0.5,0,0))
plot.new()
legend('topleft',legend=row.names(conti_coex),border="transparent",pch=15,
       col=colovec,bg=colovec,bty ="n",horiz=FALSE,cex=0.9,pt.cex=1.4,
       x.intersp=1.5,y.intersp=3.3)
par(op)
par(def.par)
...
##############################################################################/
#ex-Figure 3A: relation between HDI & organization of resistance monitoring####
##############################################################################/
...
##############################################################################/
#ex-Figure 3B: relation between IDH and PPP use type of monitoring####
##############################################################################/
