...
colnames(IDH_coex)<-c("[0.5-0.6]","[0.6-0.7]","[0.7-0.8]",
                      "[0.8-0.9]","[0.9-1]")
row.names(IDH_coex)<-c("Academic","Academic and Private",
...
layout(cbind(1,2),widths=c(8,3))  # put legend on bottom 1/8th of the chart
op<-par(mar=c(2,3,2,1))
temp<-barplot(IDH_coex,col=colovec,border=NA,axes=FALSE,
              axisnames=FALSE,space=0.7,xpd=FALSE)
axis(1,at=temp,labels=names(effectif),lwd=4,font=2,
     cex.axis=1.2,padj=0.1,xpd=TRUE,las=1)
axis(2,lwd=4,font=2,cex.axis=1.5,las=1)
#text(temp,par("usr")[1]-10,labels=names(effectif),srt=45,
#     xpd=TRUE,cex=1.5,font=2)
box(bty="l",lwd=4)
...
plot.new()
legend('center','groups',legend=row.names(IDH_coex),border="transparent",
       fill=colovec,bty ="n",horiz=FALSE,ncol=1,cex=1.5)
par(op)
