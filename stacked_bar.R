...
layout(rbind(1,2),heights=c(8,2))  # put legend on bottom 1/8th of the chart
op<-par(mar=c(2,3,2,1))
temp<-barplot(IDH_coex,col=colovec,border=NA,axes=FALSE,
              axisnames=FALSE,space=0.7,xpd=FALSE)
axis(1,at=temp,labels=names(effectif),lwd=5,font=2,
     cex.axis=1.2,padj=0.1,xpd=TRUE,las=1)
axis(2,lwd=5,font=2,cex.axis=1.5,las=1)
