...
legend(6,80,legend=c("Loss of efficacy" , "Selection pressure" , "Randomly" ),
...
graf<-barplot(VAR,
              ...
              cex.axis =1.3,cex.lab=1.8,las=1,xaxt="n",yaxt="n",bty="n",border=NA,
              space=c(rep(0.1,3),1.2,rep(0.1,2),1.2,rep(0.1,2)),font.lab=2)
axis(1,at=graf[c(2,5,8)],labels=FALSE, lwd=4)
axis(2,at=seq(0,70,10),labels=seq(0,70,10),lwd=4,las=1,font=2,cex.axis=1.1)
box(bty="l",lwd=4)

mtext(c("Electronic","Paper","Electronic\nand Paper"),at=c(1.7,6.1,10.6),line=0.4,cex=1.5,side=1,padj=1)
...
VAR<-xtabs(~data_monitoring$q_official_formalised_sampling_+data_monitoring$q_related_registration_)
...
mtext(c("Not formalised","Formalised"),at=graf[c(1,3)]+0.65,line=0.4,cex=1.5,side=1,padj=1)
legend(-0.2,72,legend=c("Impact on registration procedure :"),
       bty="n",cex=1.4,xpd=TRUE)
legend(5.5,75,legend=c("No","Yes"),
       pch=15,col=thecol,bg=thecol,bty="n",cex=1.4,pt.cex=1.6,xpd=TRUE)
mtext("Formalisation\nof sampling protocol", at=3.5, line=2.5,cex=1.8,side=1,font=2,padj=1)
...
graf<-barplot(VAR,
              col=rep(thecol,3),ylim=c(0,70),ylab="Number of responses",
              cex.axis =1.3,cex.lab=1.8,las=1,xaxt="n",yaxt="n",bty="n",border=NA,
              space=c(rep(0.1,5),1.2,rep(0.1,4),1.2,rep(0.1,4)),font.lab=2)

axis(1,at=graf[c(3,8,13)],labels=FALSE, lwd=4)
axis(2,at=seq(0,70,10),labels=seq(0,70,10),lwd=4,las=1,font=2,cex.axis=1.1)
box(bty="l",lwd=4)

mtext(c("Academic","Governmental","Private"),at=graf[c(3,8,13)],line=1.5,cex=1.5,side=1)
mtext("Type of monitoring system", at=9.4, line=3,cex=1.8,side=1,font=2,padj=1)
legend(0.5,80,legend=c("Not published","During cropping season","Yearly","Less frequently","Other"),
       pch=15,col=thecol,bg=thecol,bty="n",cex=1.3,pt.cex=1.6,xpd=TRUE,ncol=2)
par(op)
...
#calcul des quantités à partir des variables en binaires
...
#pool des Privates, Advisers, Distributors
var6<-var3+var4+var5

#création du jeu de données ordonné pour tracer le barplot
table<-data.frame(Facteur1=rep(levels(data_monitoring$type_monitoring_par_repondant),3),
                  Facteur2=c(rep("Academics",3),rep("Officials",3),rep("Privates",3)),
                  Count=c(var1,var2,var6))

#tri pour mettre academ, public, private
table<-table[c(1,4,7,3,6,9,2,5,8),]
...
mtext(levels(LEVEL)[c(1,3,2)],at=c(1.7,6.1,10.6),line=1.5,cex=1.5,side=1)
mtext("Type of monitoring system", at=6.1, line=3,cex=1.8,side=1,font=2,padj=1)
legend(6,105,legend=c("Academics" ,"Officials" , "Privates" ),
       pch=15,col=thecol,bg=thecol,bty="n",cex=1.4,pt.cex=1.6,xpd=TRUE)
par(op)
